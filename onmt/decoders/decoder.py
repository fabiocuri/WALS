...
class RNNDecoderBase(nn.Module):
    ...
    def __init__(self, wals_model, rnn_type, bidirectional_encoder, num_layers,
        ...
        if self.wals_model == 'WalstoTarget_Target' or self.wals_model == 'WalstoTarget_Both':

            self.rnn_wals = self._build_rnn(rnn_type,
                                       input_size=self.hidden_size + 2*self.wals_size,
    ...
    def forward(self, tgt, memory_bank, state, memory_lengths=None, wals_features=None, step=None):
        """
        Args:
        ...
        decoder_final, decoder_outputs, attns = self._run_forward_pass(
            tgt, memory_bank, state, memory_lengths=memory_lengths, wals_features=wals_features)
...
class StdRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, state, memory_lengths=None, wals_features=None):
        """
        ...
        if self.wals_model == 'WalstoTarget_Target' or self.wals_model == 'WalstoTarget_Both':
            ...
            if isinstance(self.rnn, nn.GRU):
                rnn_output, decoder_final = self.rnn_wals(emb, state.hidden[0])
            else:
...
class RNNDecoderStateDoublyAttentive(DecoderState):
    ...
    def __init__(self, wals_features, hidden_size, rnnstate):
        ...
        self.input_feed = self.hidden[0].data.new(*h_size).zero_() \
                              .unsqueeze(0)
        self.input_feed_wals = wals_features.data.new(*h_size).zero_() \
                              .unsqueeze(0)

    @property
    def _all(self):
        return self.hidden + (self.input_feed, self.input_feed_wals)

    def update_state(self, rnnstate, input_feed, input_feed_wals, coverage, coverage_wals):
        """ Update decoder state """
        if not isinstance(rnnstate, tuple):
            self.hidden = (rnnstate,)
        else:
            self.hidden = rnnstate
        self.input_feed = input_feed
        self.input_feed_wals = input_feed_wals
        self.coverage = coverage
        self.coverage_wals = coverage_wals
...
class RNNDecoderBaseDoublyAttentive(nn.Module):
    ...
    def forward(self, tgt, memory_bank, state, memory_lengths=None, wals_features=None, step=None):
        ...
        decoder_final, decoder_outputs, decoder_outputs_wals, attns = self._run_forward_pass(
            tgt, memory_bank, state, wals_features=wals_features, memory_lengths=memory_lengths)
        ...
        state.update_state(decoder_final,
                           final_output.unsqueeze(0),
                           final_output_wals.unsqueeze(0),
                           coverage,
                           coverage_wals)
    ...
    def init_decoder_state(self, src, memory_bank, wals_features, encoder_final,
                           with_cache=False):
        """ Init decoder state with last state of the encoder """
        def _fix_enc_hidden(hidden):
            # The encoder hidden is  (layers*directions) x batch x dim.
        ...
        if isinstance(encoder_final, tuple):  # LSTM
            return RNNDecoderStateDoublyAttentive(wals_features, self.hidden_size,
                                   tuple([_fix_enc_hidden(enc_hid)
                                          for enc_hid in encoder_final]))
        else:  # GRU
            return RNNDecoderStateDoublyAttentive(wals_features, self.hidden_size,
